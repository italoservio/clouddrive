...
func HttpHeaders(wri http.ResponseWriter, req *http.Request) *structs.HttpError {
	...
	if !accept_json {
		return structs.CreateHttpError(
			"This API only works with JSON content-type. Please set the correct 'Accept' header.",
	...
	if !sends_json {
		return structs.CreateHttpError(
			"This API only works with JSON content-type. Please set the correct 'Content-Type' header.",
			http.StatusBadRequest,
			http.StatusText(http.StatusBadRequest),
		)
	}
